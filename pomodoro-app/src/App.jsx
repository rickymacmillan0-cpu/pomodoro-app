  ...
  return (
    ...
    >
      ...
      <circle
        ...
        r={radius}
        fill="none"
        stroke={isWork ? 'rgba(255, 69, 58, 0.12)' : 'rgba(52, 199, 89, 0.12)'}
        strokeWidth={stroke}
        strokeLinecap="round"
      />
      {/* 进度弧 */}
...
export default function App() {
  ...
  const topRef = useRef(false) // 避免 state 闭包问题
  ...
  return (
    <div className="relative w-full h-full rounded-apple overflow-hidden glass-bg">
      {/* ── 自定义标题栏 ──────────────────────── */}
          ...
          <button
            ...
            className={`btn-spring w-7 h-7 rounded-full flex items-center justify-center
                       ...
                       ${alwaysOnTop
                         ...
                         : 'bg-black/[0.05] text-[#86868B] hover:bg-black/[0.10]'
                       }`}
      ...
      <div className="flex flex-col items-center justify-center h-full pt-4 pb-6 px-8">
        ...
        {/* 状态标签 */}
        <p className={`text-sm font-medium tracking-wide ${accentColor} mb-3 transition-colors duration-500`}>
        ...
        <div className="relative w-[220px] h-[220px] mb-5">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span
              className="text-[56px] font-semibold tracking-tighter text-[#1D1D1F] tabular-nums leading-none"
        ...
        <div className="flex items-center gap-4 mb-4">
          <button
            ...
            className={`btn-spring text-white text-[15px] font-medium
                       px-8 py-3 rounded-full shadow-lg shadow-black/10
          ...
          <button
            onClick={reset}
            className="btn-spring bg-black/[0.06] hover:bg-black/[0.10] text-[#1D1D1F]/70 text-[15px] font-medium
            ...
            return (
              <div
                ...
                className={`w-2.5 h-2.5 rounded-full transition-all duration-500 ${
                  isFilled ? 'bg-[#FF453A] scale-100 shadow shadow-[#FF453A]/30' : 'bg-black/[0.08] scale-90'
                }`}
              />
            )
          })}
        ...
        {/* ── 今日统计 ──────────────────────────── */}
        <p className="text-xs text-[#86868B] font-medium tracking-wide">
          今日已完成 <span className="text-[#1D1D1F] font-semibold">{totalToday}</span> 个番茄
        </p>

        {/* ── 模式切换 ──────────────────────────── */}
        <div className="mt-5 flex gap-1.5 bg-black/[0.04] p-1 rounded-full">
          {[
            ...
            <button
              ...
              className={`btn-spring w-9 h-9 rounded-full text-sm flex items-center justify-center
                         transition-all duration-300
                         ${mode === key
                           ? 'bg-white shadow-md shadow-black/8 text-base'
                           : 'text-base hover:bg-black/[0.04]'
                         }`}
            >
              {label}
            </button>
